...
import { useSpectrogramContext } from '@/pages/recording-view/hooks/use-spectrogram-context';

export const AnnotationRow = ({ annotation, index, setCurrentFFT }) => {
  const { meta, fftSize, setMeta, setSelectedAnnotation } = useSpectrogramContext();
  const data = useGetAnnotationRow(annotation, index, setCurrentFFT, meta, fftSize, setMeta, setSelectedAnnotation);

  return <>{data}</>;
};

export const useGetAnnotationRow = (
  annotation,
  index,
  setCurrentFFT,
  meta,
  ...
  const updateAnnotation = (value, parent) => {
  ...
  const calculateCurrentAnnotation = (startCapture, parent) => {
  ...
  return calculateCurrentAnnotation(meta.captures[0], null);
};
